...
static void send_usb_connect_notify(struct work_struct *w)
{
	...
	if ((board_mfg_mode() == 5 || USB_disabled )&& motg->connect_type == CONNECT_TYPE_USB) { /* off-mode charing */
...
static void ulpi_init(struct msm_otg *motg)
{
	...
	if(!IS_ERR(fp)){
		...
		set_fs(fs);
		/*XXX use too many tmp buf, find a good way to handle this*/
...
static int msm_otg_suspend(struct msm_otg *motg)
{
	...
	/* CI_45NM_INTEGRATED_PHY is for legacy projects, */
...
static void msm_otg_start_host(struct usb_otg *otg, int on)
{
	...
	/* Don't enable/disable host mode when USB_disabled be turned on (fake sleep)
	 * except USB_disabled be turned on under host mode
	 * need to disable host mode first
	 * */
	if (USB_disabled) {
		if (stop_usb_host == TRY_STOP_HOST_STATE) {
			...
			/* re-enable host mode when leave fake sleep mode */
			if (on) {
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_A_WAIT_BCON:
		...
		if (USB_disabled && stop_usb_host != STOP_HOST_STATE) {
			...
			/* usb host mode was closed by fake sleep or USB_disable attr
			 * need re-enable host mode if leave fake sleep */
